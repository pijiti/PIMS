class ExistingOrders < ActiveRecord::Migration
  def change
    ...
    ServiceRequest.where.not(:status => "PENDING").each do |s|
      o = Order.create(:number => "#{PimsConfig.find_by_property_name('order_number_prefix').property_value}-#{500 + counter}")
      o.service_requests << s
      counter += 1
      puts counter
    end
